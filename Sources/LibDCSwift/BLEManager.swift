...
public class CoreBluetoothManager: NSObject, CoreBluetoothManagerProtocol, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    @objc public func readDataPartial(_ requested: Int32) -> Data? {
        ...
        // open_ble_device() in configuredc.c retries dc_device_open up to
        // 5 times on the same BLE link with 3 s delays between attempts.
        //
        // The 5 s floor keeps the BLE link alive long enough for the
        // i300C's Bluetooth module to fully initialize.  Shorter floors
        // (2 s) leave the module half-initialized, causing subsequent BLE
        // connections to time out entirely (30 s peripheral-ready wait).
        //
    ...
    @objc public func connect(toDevice address: String!) -> Bool {
        ...
        // Wait for the peripheral to be fully disconnected before issuing a
        // new connect.  CoreBluetooth reuses CBPeripheral instances, and if
        // the previous connection is still tearing down internally the new
        // connect triggers "cannot add handler" state-machine errors that
        // delay (or prevent) the second connection.  This is critical for
        // Oceanic / Aqualung devices that need a connect-fail-reconnect cycle.
        if peripheral.state != .disconnected {
            if Logger.shared.isDebugMode {
                logDebug("[BLE CONNECT] Peripheral state is \(peripheral.state.rawValue), waiting for .disconnected")
            }
